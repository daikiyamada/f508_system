...
require_once 'Manager.php';
  try{
    ...
    $db=connect();
    $sql = 'INSERT INTO Reservation(date,class,ID,purpose) VALUES(:date,:class,:ID,:purpose)';
    //プリペアドステートメントを生成
    $stt = $db ->prepare($sql,array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
    //プリペアドステートメントを実行
    $stt->execute(array(':date'=>$_POST['date'],'class'=>$_POST['class'],':ID'=>$_POST['ID'],':purpose'=> $_POST['purpose']));
    $db = NULL;
